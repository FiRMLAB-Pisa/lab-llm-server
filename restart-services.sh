...
echo "[1/8] Restarting Ollama (main — inference, port 11434)..."
systemctl restart ollama
echo "      done."

echo "[2/8] Restarting Ollama autocomplete (starcoder2:3b, port 11435)..."
systemctl restart ollama-autocomplete
echo "      done."

echo "[3/8] Restarting Lab Knowledge MCP..."
systemctl restart lab-knowledge
echo "      done."

echo "[4/8] Restarting Lab Status dashboard..."
systemctl restart lab-status
echo "      done."

echo "[5/8] Restarting Web Search MCP..."
...
echo "[7/8] Restarting LiteLLM proxy (port 4000)..."
docker compose -f "${COMPOSE_FILE%openhands*}litellm-compose.yml" restart 2>/dev/null || true
echo "      done."

echo "[8/8] Restarting OpenHands..."
...
docker compose -f "${COMPOSE_FILE%openhands*}litellm-compose.yml" ps --format "table {{.Name}}\t{{.Status}}" 2>/dev/null || true
echo ""
